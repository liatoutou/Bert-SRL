...
def expand_to_wordpieces(original_sentence: List, tokenizer: BertTokenizer, original_labels: List = None) -> Tuple[
    List, List]:
    """
...
def data_to_tensors(dataset: List, binary_sec: List, tokenizer: BertTokenizer, max_len: int, labels: List = None,
    ...
    # PAD ALL SEQUENCES
    bin = pad_sequences(binary_sec, maxlen=max_len, dtype="long", value=0, truncating="post", padding="post")
    ...
    return LongTensor(input_ids), LongTensor(bin), LongTensor(attention_masks), label_ids, LongTensor(seq_lengths)
...
def read_json(filename: str) -> Tuple[List, List, List, Dict]:
    ''' reads input data in json format, returns lists (data, labels, binary sequence, label dictionary) to be vectorized for bert

    :param filename: path to the data file (train / dev / test)
    :return: list, list, list, dict
    '''

    all_sentences, all_labels, all_binary_seq = [], [], []
    label_dict = {}

    with open(filename, 'r', encoding='utf-8') as f:
        data = json.load(f)

    for i, sentences in data.items():
        for sent in sentences:
            all_sentences.append(sent["seq_words"])
            all_labels.append(sent["BIO"])
            label_dict = add_to_label_dict(all_labels, label_dict)
            pred_sense = sent["pred_sense"]

            if isinstance(pred_sense, list) and len(pred_sense) == 2 and isinstance(pred_sense[0],
                                                                                    int):  # Check if pred_sense is a valid list
                sense_id = pred_sense[0]
                binary_seq = [1 if idx == sense_id else 0 for idx in range(len(sent["seq_words"]))]

                # predicates.append(predicate)
                all_binary_seq.append(binary_seq)
            else:
                all_binary_seq.append([0] * len(sent["seq_words"]))

    return all_sentences, all_labels, all_binary_seq, label_dict
...
def evaluate_bert_model(eval_dataloader: DataLoader, eval_batch_size: int, model: BertModel, tokenizer: BertTokenizer,
    ...
    for batch in eval_dataloader:
        ...
        b_input_ids, b_input_mask, b_labels,b_length, b_binary = batch

        with torch.no_grad():
            outputs = model(b_input_ids, token_type_ids=b_binary, attention_mask=b_input_mask, labels=b_labels)
