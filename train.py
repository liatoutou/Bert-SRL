...
argument_parser.add_argument('-ep', '--epochs', type=int)
...
TRAIN_DATA_PATH = "data/data_train.jsonl"
...
DEV_DATA_PATH = "data/data_dev.jsonl"
...
# Load Train Dataset
train_data, train_labels, binary_sec, train_label2index = utils.read_json(TRAIN_DATA_PATH)
train_inputs, train_binary, train_masks, train_labels, seq_lengths = utils.data_to_tensors(train_data,
                                                                                          binary_sec,
                                                                                          tokenizer,
                                                                                          max_len=SEQ_MAX_LEN,
                                                                                          labels=train_labels,
...
train_data = TensorDataset(train_inputs, train_masks, train_labels, train_binary)
train_sampler = RandomSampler(train_data)
train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=BATCH_SIZE)

# Load Dev Dataset
dev_data, dev_labels, dev_binary_sec, _ = utils.read_json(DEV_DATA_PATH)
dev_inputs, dev_binary, dev_masks, dev_labels, dev_lens = utils.data_to_tensors(dev_data,
                                                                               dev_binary_sec,
                                                                               tokenizer,
                                                                               max_len=SEQ_MAX_LEN,
                                                                               labels=dev_labels,
...
dev_data = TensorDataset(dev_inputs, dev_masks, dev_labels, dev_binary)
